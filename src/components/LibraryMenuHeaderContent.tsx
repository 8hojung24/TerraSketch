...
export const LibraryDropdownMenuButton: React.FC<{
  ...
  className,
    ...
    const onLibraryImport = async () => {
      try {
        await library.updateLibrary({
          libraryItems: fileOpen({ //파일로 라이브러리 선택
          ...
          merge: true, //가져온 라이브러리 항목을 기존 항목과 병합
          openLibraryMenu: true, //업데이트 후 라이브러리 메뉴를 엶
        });
      } catch (error: any) {
    ...
    const onLibraryExport = async () => {
      ...
      saveLibraryAsJSON(libraryItems) //라이브러리 항목을 json 형식으로 저장
